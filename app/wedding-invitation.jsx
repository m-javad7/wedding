...
const weddingInfo = {
  ...
  musicFile: process.env.NEXT_PUBLIC_MUSIC_URL || '/a.mp3',
};
...
function useRomanticMusic() {
  ...
  function start() {
    try {
      if (!audioRef.current) {
        const audio = new Audio(weddingInfo.musicFile);
        audio.loop = true;
        audio.volume = 0.6;
...
function WaxSeal({ phase }) {
  ...
  return (
    <div
      className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center rounded-full transition-all duration-500"
      style={{
        bottom: '-28px',
        width: 70,
        height: 70,
        background: `radial-gradient(circle at 35% 30%, #f5edd6, #d8b978 40%, #b8923e 75%, #8a6d2b 100%)`,
        boxShadow: `
          0 0 0 6px rgba(255,253,246,0.4),
          0 0 0 12px rgba(201,162,75,0.12),
          0 15px 40px rgba(76,55,20,0.5),
          inset 0 -6px 10px rgba(0,0,0,0.25),
          inset 0 6px 12px rgba(255,255,255,0.25)
        `,
        opacity: isOpen ? 0 : 1,
        transform: `translateX(-50%) scale(${isOpen ? 0.1 : 1}) rotate(${isOpen ? '45deg' : '0deg'})`,
        animation: phase === 'closed' ? 'sealPulse 3s ease-in-out infinite' : 'none',
        zIndex: 10,
    ...
    >
      ...
      <div className="absolute inset-0 rounded-full overflow-hidden">
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(circle at 60% 35%, rgba(255,255,255,0.25), transparent 60%)',
        }} />
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(circle at 30% 70%, rgba(0,0,0,0.12), transparent 50%)',
        }} />
        ...
        <div className="absolute top-[20%] left-[30%] w-[2px] h-[8px] rounded-full" style={{
          background: 'rgba(139,109,43,0.3)',
          transform: 'rotate(-20deg)',
        }} />
        <div className="absolute bottom-[25%] right-[25%] w-[1.5px] h-[6px] rounded-full" style={{
          background: 'rgba(139,109,43,0.25)',
      ...
      {[0, 60, 120, 180, 240, 300].map((deg, i) => (
        <span key={i} className="absolute rounded-full" style={{
          ...
          left: '50%',
          top: '50%',
          transform: `rotate(${deg}deg) translateX(-50%) translateY(-${28}px)`,
        }} />
      ))}
  ...
  const handleSend = (method) => {
    ...
    switch(method) {
      ...
      case 'whatsapp':
        ...
        break;
      case 'ita':
        // لینک ایتا - با توجه به ساختار ایتا
        window.open(`https://eitaa.com//${weddingInfo.itaUsername || 'm_javad7721'}?text=${encoded}`, '_blank');
        break;
      default:
        break;
    }
  ...
  return (
    <div dir="rtl" className="relative w-full min-h-screen overflow-x-hidden" style={{ fontFamily: "'Vazirmatn', Tahoma, sans-serif" }}>
      <style>{`
        ...
        @keyframes sealPulse {
          0%, 100% {
            box-shadow: 0 0 0 0 rgba(201,162,75,0.35), 0 15px 40px rgba(76,55,20,0.5), inset 0 -6px 10px rgba(0,0,0,0.25), inset 0 6px 12px rgba(255,255,255,0.25);
          }
          50% {
            box-shadow: 0 0 0 25px rgba(201,162,75,0), 0 15px 40px rgba(76,55,20,0.5), inset 0 -6px 10px rgba(0,0,0,0.25), inset 0 6px 12px rgba(255,255,255,0.25);
          }
        }
        ...
        @media (max-width: 640px) {
          .envelope-container { transform: scale(0.9); }
        }
      `}</style>
      ...
      <div className="relative min-h-screen w-full flex items-center justify-center p-4 sm:p-6 overflow-hidden" style={{
        ...
        {/* پس‌زمینه‌های آمبیانت با طراحی بهتر */}
        ...
        {/* دکمه قطع/وصل موسیقی با طراحی بهتر */}
        {phase === 'open' && (
          <button
